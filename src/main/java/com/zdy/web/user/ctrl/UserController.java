...
import com.zdy.web.user.vo.UserVO;
...
public class UserController extends BaseController {
	...
	public Result login(UserVO userVO, String remember, String validateCode, HttpServletResponse response) throws MyException {
		...
		try {
			...
			req.setPassword(MD5coding.MD5(userVO.getPassword()));
			...
			if (serviceResult != null && serviceResult.isSuccess() && serviceResult.getBusinessObject() != null
				...
				setCookie(serviceResult.getBusinessObject(), remember, response, userVO.getPassword());
			} else {
	...
	private void setSession(UserResp userResp) throws MyException {
		...
		user.setUserCompellation(userResp.getUserCompellation());
	...
	private void setCookie(UserResp user, String remember, HttpServletResponse response, String pwd) {
		if ("true".equals(remember)) {
			...
			Cookie pwdCookie = new Cookie("zdy_operator_login_password", pwd);
